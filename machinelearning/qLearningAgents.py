...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        next_states_np = np.vstack([x.next_state for x in minibatch])
        next_states = nn.Constant(next_states_np)
        ...
        Q_next_predict = network.run(next_states).data
        Q_next_target_predict = target_network.run(next_states).data

        for s, a, r, done, target_vals, values, target_values \
             in zip(states_np, actions, rewards, dones, Q_target, Q_next_predict, Q_next_target_predict):
            exploration_bonus = 1 / (2 * np.sqrt(self.counts[s[0], s[1]] / 100))
            best_action_index = np.argmax(values)
            best_value = target_values[best_action_index]
            target_vals[a] = r + ((1 - done) * self.discount * best_value) + exploration_bonus
            new_Q_target.append(target_vals)

        Q_target = np.array(new_Q_target)
